...
\subsection[Product Perspective]{\hyperlink{toc}{Product Perspective}}
	...
	The static analysis now continues to define the internal structure of the system, in particular a high-level class diagram that considers the most important objects and their relations in order to achieve the \hyperref[sec:goals]{\textcolor{blue}{goals}} is shown in \autoref{fig:classDiagram}.\\
	...
	\label{sec:stateDiagrams}
		...
		\paragraph{Request}
			Information retrieval is another critical functionality that needs a state diagram to define how the information to be mined is provided with different levels of visibility depending on the role of the customer. The diagram (\autoref{fig:requestState}) coincides for both different kinds of requests a customer can do, in fact it is used to highlight the different visibility of the answered information rather than the difference between these requests. Whenever a query is received, the system has first to retrieve the data that is required thanks to the filters provided by the customer. After the information is found in the \textit{processed} state, the application needs to recognize the user in order to choose if answer with the retrieved data or not. If the query comes from a customer who could perform it the data is \textit{accepted} and then sent back reaching the \textit{data sent} state, otherwise it is \textit{rejected} and the answer will never reach its inquiring. 
...
\subsection[Product Functions]{\hyperlink{toc}{Product Functions}}
	...
	\subsubsection[Statistics Function]{\hyperlink{toc}{Mining Function}}
		...
		It is important to underline the fact that data is provided to each customer depending on its role. \textbf{Different levels of visibility} have to be considered when providing the information retrieved by the two shared functionalities. For example we should consider that the authorities have always the option to retrieve their information also with the number of the plates of the violations or the names of the users who reported them. The \emph{Single Violation Search} gives also to authorities a way to retrieve violations in the most specific way they could.
	
	\subsubsection[Safety Function]{\hyperlink{toc}{Safety Function}}
		\label{sec:safetyFunction}
		SafeStreets is able to process the information provided by the municipality about the accidents that occur in a territory. Thanks to this additional data the system can cross it with its stored one and determine the \textbf{safety} of each street as we now precisely describe.\\
		...
		The safety of each street and then the related intervention is determined each day with the stored information about the previous month. Thanks to this data retrieved by the \emph{crossing} process, SafeStreets is also able to provide the following functionalities:
	 	...
	 	Both these services are provided to the customers who can also decide to filter the information retrieved by choosing one of the following filters: \textbf{city, path} or \textbf{street.} 
...
\subsection[User Characteristics]{\hyperlink{toc}{User Characteristics}}
	...
	\begin{itemize}
		\item \textbf{USER}
			\begin{itemize}
				\item Is considered to be reliable but human (he can insert the wrong plate).
		...
		\item \textbf{AUTHORITY/MUNICIPALITY}
			\begin{itemize}
				...
				\item Can register to SafeStreets in order to be recognized as such.
				\item Must login to SafeStreets to benefit of its services.
				\item Can search for single violations.
				\item Can filter the mined data.
...
\subsection[Domain Assumptions]{\hyperlink{toc}{Domain Assumptions}}
	...
	\begin{enumerate}[label=\textbf{DA\arabic*}]
		...
		\item Users are reliable but human.
		\item Information provided by the municipality is correct and reliable.
		%todo add all other domain assumptions
	\end{enumerate} 
...
\subsection[The World and the Machine]{\hyperlink{toc}{The World and the Machine}}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=\textwidth, scale=0.3]{WorldAndMachine.png}
