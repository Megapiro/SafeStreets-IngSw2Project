...
\subsection[Product Perspective]{\hyperlink{toc}{Product Perspective}}
	...
	\subsubsection[System Interfaces]{\hyperlink{toc}{System Interfaces}}
		...
		In this section we are going to deal with the interfaces that SafeStreets needs to \emph{use} in order to provide its functionalities. The definition of SafeStreet's interfaces to the external world, instead, will be described in the further related section (\blueRef{sec:externalInterfaceRequirements}), while precise details are given in the Design Document \cite{DD}.\\
	
		To accomplish the \hyperref[sec:goals]{\textbf{goals}} stated in the introduction the application interfaces with three main external systems as reported in the following picture (\blueAutoref{fig:systemInterfaces}).  
		\vspace{0,3cm}
